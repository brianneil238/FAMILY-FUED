...
$(document).ready(function () {
    ...
    function updateLeaderboard() {
        $('#team1').text(leaderboard.team1);
        $('#team2').text(leaderboard.team2);
    }

    // Prevent spam: disable buttons after clicking
    function disableButtons() {
        $('#awardTeam1, #awardTeam2, #newQuestion').prop('disabled', true);
    }

    // Enable buttons for the next round
    function enableButtons() {
        $('#awardTeam1, #awardTeam2, #newQuestion').prop('disabled', false);
    }

    // Update the team scores
    function awardPoints(team) {
        let currentScore = parseInt($('#boardScore').text());
        leaderboard[team] += currentScore;
        updateLeaderboard();
        $('#boardScore').text(0);
    }

    // Event listeners for awarding points
    $('#awardTeam1').on('click', function () {
        awardPoints('team1');
        disableButtons();
    });

    $('#awardTeam2').on('click', function () {
        awardPoints('team2');
        disableButtons();
    });
    ...
    $('#startGame').on('click', function () {
        // Hide welcome screen and show game board
        $('#welcomeScreen').hide();
        app.init();
    });
